...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!event || event.organization.slug !== slug) {
    return {
  ...
  return {
    title: `${event.name} | ${event.organization.name} | Festoryx`,
    description: event.shortDescription || `Join ${event.name} run by ${event.organization.name} on Festoryx.`,
  };
}

export default async function OrgEventDetailPage({ params, searchParams }: PageProps) {
  const { slug, eventSlug } = await params;
  const resolvedSearchParams = await searchParams;
  const dbEvent = await getEventBySlug(eventSlug);

  if (!dbEvent || dbEvent.organization.slug !== slug) {
    notFound();
  }
    ...
    <div className="flex min-h-screen flex-col bg-[#030014]">
      ...
      <main className="flex-grow pt-28 pb-16">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <Link
            href={`/org/${slug}`}
            className="inline-flex items-center gap-2 text-sm text-indigo-400 hover:text-indigo-300 mb-8 transition-colors duration-200"
          >
            <ArrowLeft className="h-4 w-4" />
            <span>Back to {org.name} profile</span>
          </Link>

          {/* Banner / Title Area */}
          <div className="relative overflow-hidden rounded-3xl border border-white/10 bg-[#060317] p-8 backdrop-blur-md shadow-2xl mb-8 min-h-[220px] flex items-end">
            {event.bannerUrl ? (
              <>
                <img
                  src={event.bannerUrl}
                ...
                />
                <div className="absolute inset-0 bg-gradient-to-t from-[#030014] via-black/20 to-black/40" />
              </>
            ) : (
              <div className="absolute -right-20 -top-20 h-60 w-60 rounded-full bg-indigo-500/10 blur-3xl" />
            )}
            <div className="relative z-10 w-full">
              <span className="inline-block px-2.5 py-0.5 bg-indigo-500/10 border border-indigo-500/30 text-indigo-400 text-xs rounded-full uppercase tracking-wider font-semibold mb-3">
                {org.name}
              </span>
          ...
          {(event.winner1 || event.winner2 || event.winner3) && (
            <div className="rounded-2xl border border-amber-500/30 bg-amber-500/10 p-6 backdrop-blur-md shadow-xl mb-8 flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          <div className="grid gap-8 lg:grid-cols-3">
            ...
            <div className="lg:col-span-2 space-y-6 min-w-0">
              ...
              <div className="flex border-b border-white/10 gap-4 overflow-x-auto pb-px no-scrollbar max-w-full">
                {tabs.map((tab) => (
                  <Link
                    key={tab.id}
                    href={`/org/${slug}/events/${event.slug}?tab=${tab.id}`}
                    scroll={false}
              ...
              {/* Tab Contents */}
              <div className="rounded-2xl border border-white/10 bg-[#060317] p-6 backdrop-blur-md shadow-xl text-gray-300 break-words">
              ...
              <div className="rounded-2xl border border-white/10 bg-[#060317] p-4 sm:p-6 backdrop-blur-md shadow-xl">
                ...
                {event.problemReleaseTime && !isProblemReleased ? (
                  <div className="flex flex-col items-center justify-center py-6 px-2 sm:p-8 text-center text-gray-400">
                    ...
                    <div>
                      ...
                      <div className="my-4 rounded-2xl border border-indigo-500/30 bg-indigo-500/10 p-3 sm:p-5 shadow-lg shadow-indigo-500/10">
                  ...
                  <div className="space-y-4 text-gray-300">
                    ...
                    <div className="pt-4 border-t border-white/5">
                      ...
                      <div className="mt-2 rounded-xl bg-black/30 p-4 font-mono text-sm overflow-x-auto whitespace-pre-wrap border border-white/5 hover:border-indigo-500/30">
            ...
            <div className="space-y-6">
              <div className="rounded-2xl border border-white/10 bg-[#060317] p-6 backdrop-blur-md shadow-xl space-y-6">
                ...
                {event.isRegistrationOpen ? (
                  <Link
                    ...
                    className="flex h-12 w-full items-center justify-center rounded-xl bg-[#9382ff] hover:bg-[#816eff] text-sm font-semibold text-white shadow-lg shadow-[#9382ff]/20 transition-all duration-300 hover:scale-[1.01]"
                  >
              ...
              {/* Payment Instructions from OrgSettings */}
              {fee > 0 && settings?.paymentInstructions && (
                <div className="rounded-2xl border border-white/10 bg-[#060317] p-6 backdrop-blur-md shadow-xl space-y-3">
